...
This algorithm can infact be simulated by a simple logic circuit. 


% \section{Lotz left bro}
...
% For this method to work, \(E(x)\) is not divisible by \(C(x)\). But is this the case
...
%     To see why think of a divisor for this operation 
%     \begin{align}
%         C(x) D(x) &= x^i \\
...
%     Thus \[E(x) = x^{i + l -1} + x^{x + l -2} + \dots + x^i\], which can also be 
...
% Possible methods:
% \begin{itemize}
%     \item \textbf{Frequency-Based:} Will different frequencies help? How to allocate different frequencies to devices?
...
%     token release message is sent if a particular device wants to obtain the message and then communicate a message. This device waits 
%     until it receives the token. 
% \end{itemize}

% The second one is a better option since it completely negates interference. A smart leader can also 
